        ...
        if imagePath:
            ...
            if ok:
                if imagePath.lower().endswith(".cr3"):
                    im = Image3(imagePath)
                    img = imageio.imread(io.BytesIO(im.jpeg_image))
                else:
                    img = imageio.imread(imagePath)
                img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                preview = ImageViewer(ndarrayToQImage(img))
                i = len(self.previews)
...
class ImageItem(QWidget):
    def __init__(self, qimage=None):
        ...
        self.discardBut.setIcon(self.style().standardIcon(QStyle.SP_DialogDiscardButton))
